...
public class BulletCasingItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> tooltips, ITooltipFlag flag) {
        ...
        if (!Screen.hasShiftDown()) {
            tooltips.add(new TranslationTextComponent("tooltip.metroguns.hold_shift_key", new StringTextComponent("[SHIFT]").withStyle(TextFormatting.YELLOW)));
        } else {
            tooltips.add(new TranslationTextComponent("tooltip.metroguns.bullet_casing_info").withStyle(TextFormatting.RED));
        }
    }
}
